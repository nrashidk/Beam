...
export default function CreateInvoice() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setOriginalSuggestions(list.filter((inv) => inv.status !== "CANCELLED"));
  ...
  useEffect(() => {
    const autoPopulateFromOriginalInvoice = async () => {
      ...
      try {
        const response = await apiClient.get(`/invoices/${formData.preceding_invoice_id}`);
                ...
                <div className="relative">
                  <input
                    ...
                    value={
                      ...
                      formData.preceding_invoice_id
                        ? (originalInvoices.find(i => i.id === formData.preceding_invoice_id)?.invoice_number || originalSearchQuery)
                        : originalSearchQuery
                    }
                    onChange={(e) => {
                      setOriginalSearchQuery(e.target.value);
                      setFormData({ ...formData, preceding_invoice_id: "" });
                    }}
                    onFocus={() => {
                      if (originalSuggestions.length > 0) setShowOriginalSuggestions(true);
                    }}
                    onKeyDown={(e) => {
                      if (e.key === 'ArrowDown') {
                        e.preventDefault();
                        setOriginalHighlight((h) => Math.min(h + 1, originalSuggestions.length - 1));
                      } else if (e.key === 'ArrowUp') {
                        e.preventDefault();
                        setOriginalHighlight((h) => Math.max(h - 1, 0));
                      } else if (e.key === 'Enter') {
                        if (originalHighlight >= 0 && originalSuggestions[originalHighlight]) {
                          const sel = originalSuggestions[originalHighlight];
                          setFormData({ ...formData, preceding_invoice_id: sel.id });
                    ...
                    placeholder={loadingOriginalInvoices ? "Loading invoices..." : "Type invoice number or customer name"}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  />

                  {showOriginalSuggestions && (originalSuggestions.length > 0 ? (
                      ...
                      {originalSuggestions.map((inv, idx) => (
                        <li
                          ...
                          onMouseDown={(e) => {
                            ...
                            e.preventDefault();
                            setFormData({ ...formData, preceding_invoice_id: inv.id });
                        ...
                        >
                          <div className="text-sm font-medium text-gray-900">{inv.invoice_number} — {inv.customer_name}</div>
                          <div className="text-xs text-gray-500">{inv.currency_code} {inv.total_amount.toFixed(2)} • {new Date(inv.issue_date).toLocaleDateString('en-AE')}</div>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    originalSearchQuery ? (
                      <div className="absolute z-50 left-0 right-0 bg-white border border-gray-200 rounded-md mt-1 p-3 text-sm text-gray-500">No matches</div>
                    ) : null
                  ))}
                </div>
              </div>
