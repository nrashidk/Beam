...
def create_invoice(
    ...
    if company.trial_start_date and company.trial_status in ["ACTIVE", "EXPIRED"]:
        ...
        if company.free_plan_type == "DURATION":
            ...
            expired_message = (
                f"Free trial expired ({trial_duration_days} days). Please subscribe to continue creating invoices."
            )
        elif company.free_plan_type == "INVOICE_COUNT":
            trial_expired_now = trial_invoices_remaining == 0
            expired_message = (
                f"Free trial expired ({invoice_limit} invoices used). Please subscribe to continue creating invoices."
            )
        else:
            trial_expired_now = (
