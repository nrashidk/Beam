...
class BulkImportValidator:
    ...
    def generate_invoice_template() -> pd.DataFrame:
        ...
        template_data = {
            'issue_date': ['2025-01-15', '2025-01-16', '2025-01-17', '2025-01-18', '2025-01-19'],
            'due_date': ['2025-02-15', '2025-02-16', '2025-02-17', '2025-02-18', '2025-02-19'],
            ...
            'invoice_type': ['COMMERCIAL', 'TAX_INVOICE', 'CREDIT_NOTE', 'TAX_CREDIT_NOTE', '380'],
            'customer_name': ['ABC Trading LLC', 'XYZ Company', 'DEF Corporation', 'GHI Enterprises', 'JKL Traders'],
            'customer_email': ['customer@example.com', 'customer2@example.com', 'customer3@example.com', 'customer4@example.com', 'customer5@example.com'],
            'customer_trn': ['', '', '', '', ''],  # Optional for all
            'customer_address': ['Dubai, UAE', 'Abu Dhabi, UAE', 'Sharjah, UAE', 'Ajman, UAE', 'Ras Al Khaimah, UAE'],
            'customer_city': ['Dubai', 'Abu Dhabi', 'Sharjah', 'Ajman', 'Ras Al Khaimah'],
            'preceding_invoice_number': ['', '', 'CI-00001', '', ''],  # Required for credit notes only
            'item_name': ['Goods', 'Services', 'Refund', 'License', 'Materials'],
            'item_description': ['Consulting Services', 'Professional work', 'Refund for returned goods', 'Software License', 'Building materials'],
    ...
    def validate_invoice_file(file_content: bytes, filename: str) -> Tuple[bool, List[Dict[str, Any]], List[str]]:
        ...
        try:
            ...
            required_columns = [
                ...
                'customer_name', 'item_description',
                'quantity', 'unit_price'
            ]
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    ...
                    trn_value = str(trn_raw).strip() if trn_raw and not pd.isna(trn_raw) else ''
                    if trn_value and (len(trn_value) != 15 or not trn_value.isdigit()):
                        row_errors.append(f"Row {row_num}: If provided, TRN must be 15 digits (numeric only)")
                    
                    customer_name_raw = row.get('customer_name', '') if 'customer_name' in row else ''
                    if pd.isna(customer_name_raw) or str(customer_name_raw).strip() == '':
                        row_errors.append(f"Row {row_num}: Customer name is required")
                    
                    try:
                        quantity_raw = row.get('quantity', 0) if 'quantity' in row else 0
                        quantity = float(quantity_raw) if not pd.isna(quantity_raw) else 0
                        if quantity <= 0:
                    ...
                    try:
                        unit_price_raw = row.get('unit_price', 0) if 'unit_price' in row else 0
                        unit_price = float(unit_price_raw) if not pd.isna(unit_price_raw) else 0
                        if unit_price < 0:
                    ...
                    if has_preceding_invoice:
                        preceding_raw = row.get('preceding_invoice_number', '') if 'preceding_invoice_number' in row else ''
                    ...
                    issue_date_str = None
                    issue_date_raw = row.get('issue_date') if 'issue_date' in row else None
                    if not pd.isna(issue_date_raw):
                        try:
                            if isinstance(issue_date_raw, (datetime, date, pd.Timestamp)):
                    ...
                    if row_errors:
                        ...
                        invoice_data = {
                            'row_num': int(row_num),
                            'invoice_number': safe_str(row.get('invoice_number')) or None,
                            'issue_date': issue_date_str,
                            'due_date': due_date_str,
                            'invoice_type': invoice_type,
                            'preceding_invoice_number': preceding_invoice_number,
                            'customer_name': safe_str(row.get('customer_name')),
                            'customer_email': safe_str(row.get('customer_email')) or None,
                            'customer_trn': trn_value or None,
                            'customer_address': safe_str(row.get('customer_address')) or None,
                            'customer_city': safe_str(row.get('customer_city')) or None,
                            'item_name': safe_str(row.get('item_name'), 'Item') or 'Item',
                            'item_description': safe_str(row.get('item_description')),
                            'quantity': float(quantity_raw) if not pd.isna(quantity_raw) else 0,
                            'unit_price': float(unit_price_raw) if not pd.isna(unit_price_raw) else 0,
                            'unit_name': safe_str(row.get('unit_name'), 'Unit') or 'Unit',
                            'tax_category': safe_str(row.get('tax_category'), 'S') or 'S',
                            'tax_percent': float(row.get('tax_percent', 5)) if not pd.isna(row.get('tax_percent')) else 5.0,
                            'tax_code': safe_str(row.get('tax_code'), 'S_5_5') or 'S_5_5'
                        }
    ...
    def validate_vendor_file(file_content: bytes, filename: str) -> Tuple[bool, List[Dict[str, Any]], List[str]]:
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                try:
                    vendor_name_raw = row.get('vendor_name', '') if 'vendor_name' in row else ''
                    if pd.isna(vendor_name_raw) or str(vendor_name_raw).strip() == '':
                        row_errors.append(f"Row {row_num}: Vendor name is required")
                    
                    vendor_trn_raw = row.get('vendor_trn', '') if 'vendor_trn' in row else ''
                    vendor_trn_value = str(vendor_trn_raw).strip() if vendor_trn_raw and not pd.isna(vendor_trn_raw) else ''
                    ...
                    vendor_email_raw = row.get('vendor_email', '') if 'vendor_email' in row else ''
                    ...
                    if row_errors:
                        ...
                        vendor_data = {
                            ...
                            'vendor_address': safe_str(row.get('vendor_address')) or None,
                        }
