...
╚══════════════════════════════════════════════════════════════════╝
    ...
    def send_email(self,
        ...
        try:
            ...
            print(f"🚀 Calling AWS SES send_email API...")
            # Send email
            kwargs = {
                'Source': source,
                'Destination': {
            ...
            print(f"✅ AWS SES send_email successful!")
            ...
            # Safely extract error message from boto3 ClientError
            try:
            ...
            if "Unknown" in str(e) or "max retries" in str(e).lower():
                print(f"⚠️  TROUBLESHOOTING:")
                print(f"   ├─ Possible Cause 1: AWS SES is in SANDBOX mode")
                print(
                    f"   │  → Solution: Verify recipient email '{to_email}' in AWS SES console"
                )
                print(f"   │  → Or: Request production access for AWS SES")
                print(f"   ├─ Possible Cause 2: Network/connectivity issues")
                print(f"   │  → Check: Server can reach AWS SES endpoints")
                print(f"   ├─ Possible Cause 3: Invalid sender email")
                print(
                    f"   │  → Check: '{self.sender_email}' is verified in AWS SES"
                )
                print(
                    f"   └─ Possible Cause 4: AWS credentials/permissions issue"
                )
                print(
                    f"      → Check: IAM permissions include 'ses:SendEmail'")
    ...
    def send_verification_email(self, to_email: str, company_name: str,
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    def send_mfa_otp_email(self, to_email: str, user_name: str,
                           otp_code: str) -> dict:
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    ...
    def send_approval_notification(
            ...
            company_name: str,
            status: str,
            admin_message: Optional[str] = None) -> dict:
