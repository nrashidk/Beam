...
- **UAE FTA Accreditation Compliance:** Achieved high readiness across T1-T32 requirements, including General Ledger, FAF generation with `SupplyDate` in both SalesData and PurchaseData, VAT Return generation (Box 2 credit notes, Box 10/11 expenses), Audit Trail, security, invoice locking, debit notes, supply dates, concurrent update prevention (optimistic locking on `CompanyDB`/`InwardInvoiceDB`/`VATReturnDB` via `If-Match` header → HTTP 409 on version mismatch), periodic reports, data archival, bulk archive XLSX export (`GET /invoices/archive/export`), in-application database backup (`POST /admin/backup/trigger`), and gapless sequential invoice numbering via `invoice_sequences` table with atomic `INSERT … ON CONFLICT DO UPDATE RETURNING` per `(company_id, prefix)` pair. April 2026 gap-analysis fixes: (1) TRN truncation bug fixed in `ubl_generator.py` — full 15-digit TRN now emitted; (2) Universal `<cac:PaymentMeans>` added to all UBL invoices (PEPPOL BIS 3.0 mandatory); (3) Line-level `discount_amount` added to `InvoiceLineItemDB` with `<cac:AllowanceCharge>` emitted in UBL XML when discount > 0; (4) `exchange_rate` added to `InvoiceDB` with `<cac:TaxExchangeRate>` emitted for foreign-currency invoices (FTA Article 60).
