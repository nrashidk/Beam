...
- **UAE FTA Accreditation Compliance:** Achieved high readiness across T1-T32 requirements, including General Ledger, FAF generation with `SupplyDate` in both SalesData and PurchaseData, VAT Return generation (Box 2 credit notes, Box 10/11 expenses), Audit Trail, security, invoice locking, debit notes, supply dates, concurrent update prevention (optimistic locking on `CompanyDB`/`InwardInvoiceDB`/`VATReturnDB` via `If-Match` header → HTTP 409 on version mismatch), periodic reports, data archival, bulk archive XLSX export (`GET /invoices/archive/export`), in-application database backup (`POST /admin/backup/trigger`), and gapless sequential invoice numbering via `invoice_sequences` table with atomic `INSERT … ON CONFLICT DO UPDATE RETURNING` per `(company_id, prefix)` pair. April 2026 gap-analysis fixes (round 1): (1) TRN truncation bug fixed in `ubl_generator.py` — full 15-digit TRN now emitted; (2) Universal `<cac:PaymentMeans>` added to all UBL invoices (PEPPOL BIS 3.0 mandatory); (3) Line-level `discount_amount` added to `InvoiceLineItemDB` with `<cac:AllowanceCharge>` emitted in UBL XML when discount > 0; (4) `exchange_rate` added to `InvoiceDB` with `<cac:TaxExchangeRate>` emitted for foreign-currency invoices (FTA Article 60). Round 2 fixes: (5) UBL `<cac:TaxSubtotal>` now emits per-category (S/Z/E/AE/O) by grouping line items — hardcoded 'S' removed; (6) QR code API endpoint now encodes FTA TLV Base64 payload (Seller Name, TRN, Date, Total, VAT) instead of plain URL; (7) Invoice signing cert serial reads from `crypto.cert_serial` instead of hardcoded "MOCK-CERT-001"; (8) `ubl_generator.py` CustomizationID updated to `urn:peppol:pint:billing-1@ae-1`; (9) CORS restricted to production domains (involinks.ae) + dev Replit domain via `ALLOWED_ORIGINS` env var; (10) IP-based sliding-window rate limiting middleware added — 10 req/min on `/auth/login`, 5 req/min on `/auth/register` and `/auth/mfa/verify`.
