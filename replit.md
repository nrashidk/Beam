...
- **Registration & Subscription:** Comprehensive signup collecting company information (legal name, phone, email, website, TRN), complete address details (address_line1, address_line2, city, emirate, po_box), and password in an organized three-section form (Company Information, Address Information, Security). Business admins can update details via in-dashboard edit functionality after approval. Includes email verification, token-based workflow, automatic Free tier assignment, and flexible free plan configuration.
